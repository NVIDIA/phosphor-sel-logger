...
inline static void sendWatchdogEventLog(
  ...
  try {
    sdbusplus::message_t getWatchdogStatusResp = conn->call(getWatchdogStatus);
  ...
  if (!expireAction) {
    ...
    if (getExpireAction != watchdogStatus.end()) {
      ...
      expireAction->remove_prefix(
          std::min(expireAction->find_last_of(".") + 1, expireAction->size()));
    }
  }
  ...
  if (getPreTimeoutInterrupt != watchdogStatus.end()) {
    ...
    preTimeoutInterrupt.remove_prefix(std::min(
        preTimeoutInterrupt.find_last_of(".") + 1, preTimeoutInterrupt.size()));
  }
  if (preTimeoutInterrupt == "None") {
    eventData[1] &= (static_cast<uint8_t>(watchdogInterruptTypeOffsets::none)
                     << interruptTypeBits);
    ...
    eventData[1] &=
        (static_cast<uint8_t>(watchdogInterruptTypeOffsets::messageInterrupt)
         << interruptTypeBits);
  }
