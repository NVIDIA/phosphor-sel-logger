...
startPulseEventMonitor(std::shared_ptr<sdbusplus::asio::connection> conn) {
  auto pulseEventMatcherCallback = [conn](sdbusplus::message_t &msg) {
    ...
    auto variant = std::get_if<std::string>(&propertiesChanged.begin()->second);
    ...
    if (event == "CurrentHostState") {
      ...
      std::string_view hostObjPathPrefix = "/xyz/openbmc_project/state/host";
...
#else
      sd_journal_send("MESSAGE=%s", journalMsg.c_str(), "REDFISH_MESSAGE_ID=%s",
