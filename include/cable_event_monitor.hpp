...
startCableAssertMonitor(std::shared_ptr<sdbusplus::asio::connection> conn) {
  auto cableAssertMatcherCallback = [conn](sdbusplus::message_t &msg) {
    ...
    if (cableStatus) {
      eventData[0] = static_cast<uint8_t>(cableStatusEvent::assertedEvent);
    } else {
      eventData[0] = static_cast<uint8_t>(cableStatusEvent::deassertedEvent);
    }
    ...
    std::string journalMsg(std::string(sensorName) + " sensor is now " +
                           statusString);
