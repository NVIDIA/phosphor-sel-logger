...
struct DBusInternalError final : public sdbusplus::exception_t {
  ...
  const char *description() const noexcept override { return "internal error"; }
  const char *what() const noexcept override {
    return "org.freedesktop.DBus.Error.Failed: "
           "internal error";
  }

  int get_errno() const noexcept override { return EACCES; }
};
...
static void saveClearSelTimestamp() {
  int fd = open("/var/lib/ipmi/sel_erase_time", O_WRONLY | O_CREAT | O_CLOEXEC,
                0644);
  if (fd < 0) {
...
static void selDeleteRecord(const uint16_t &recordId) {
  ...
  // Add to backup file
  std::ofstream nextRecordStream(selLogDir / nextRecordFilename, std::ios::app);
...
static void writeSELEvent(const unsigned int &recordId,
  ...
  std::string selStr =
      getSELEventStr(recordId, selDataStr, genId, path, assert);
  if (selStr.empty()) {
  ...
  if (maxSELEntriesReached && !isLinearSELPolicy()) {
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>(
        "Remove first SEL event on reaching max SEL entries in circular "
        "config");
...
#endif
{
  ...
  try {
    ...
    std::cerr << "Failed to create D-Bus log entry for SEL, ERROR=" << e.what()
              << "\n";
  }
#else
